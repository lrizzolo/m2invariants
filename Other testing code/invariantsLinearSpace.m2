...
--currently, this code only works with three variables 
-- (i.e. numVars = 3 and variables x_1, x_2, and x_3)
numVars = 3
R = QQ[x_1..x_numVars]
...
-- I compute the whole minimal generating set here 
-- just so that I can take one of the elements from the set
-- Then, I can use that one element to regenerate 
-- the rest of the set in the array M.
S1 = invariants T
S1 = sort S1
...
-- there's two x_1's in this array to make it 
-- so that array indexing (which starts at 0) lines
-- up with the variable indexing (which starts at 1)
modVars := apply({x_1, x_1, x_2, x_3}, m -> sub(m, ring T) )

-- this bit of code finds the first element of S1 
-- that isn't a pure power, 
-- which will be a generator of the set
-- Quick edit: a higher degree term of S1 seems 
-- to work better in the Z9 x Z9 example, so maybe start at the end of the list?
...
while (i < modDegree) do ( 
    ...
    -- FG: issue might be here, degree can be >=
   while (j <= numVars) do (
